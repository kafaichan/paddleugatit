...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        """ Trainer """
        self.G_optim = fluid.optimizer.AdamOptimizer(learning_rate=self.lr, 
            parameter_list=itertools.chain(self.genA2B.parameters(), self.genB2A.parameters()), 
        ...
        self.D_optim = fluid.optimizer.AdamOptimizer(learning_rate=self.lr,
            parameter_list=itertools.chain(self.disGA.parameters(), self.disGB.parameters(), self.disLA.parameters(), self.disLB.parameters()), 
            beta1=0.5, beta2=0.999, regularization=fluid.regularizer.L2Decay(self.weight_decay))
    ...
    def train(self):
        ...
        with fluid.dygraph.guard(place):
            ...
            for step in range(start_iter, self.iteration + 1):
                ...
                #print(fake_A2B2A.shape)
                #print("-----------------------")
                fake_B2A2B, _, _ = self.genA2B(fake_B2A)
                #print(fake_B2A2B.shape)
                #print("-----------------------")

                fake_A2A, fake_A2A_cam_logit, _ = self.genB2A(real_A)
                #print(fake_A2A.shape)
                #print("-----------------------")                
                fake_B2B, fake_B2B_cam_logit, _ = self.genA2B(real_B)
                #print(fake_B2B.shape)
                #print("-----------------------")
                ...
                #print(G_loss_A.numpy())
                G_loss_B = self.adv_weight * (G_ad_loss_GB + G_ad_cam_loss_GB + G_ad_loss_LB + G_ad_cam_loss_LB) + self.cycle_weight * G_recon_loss_B + self.identity_weight * G_identity_loss_B + self.cam_weight * G_cam_loss_B
                #print(G_loss_B.numpy())
    ...
    def load(self, dir, step):
        genA2B_param = fluid.dygraph.load_dygraph(os.path.join(dir, 'gena2b_params_%07d' % step))
        genB2A_param = fluid.dygraph.load_dygraph(os.path.join(dir, 'genb2a_params_%07d' % step))
        disGA_param = fluid.dygraph.load_dygraph(os.path.join(dir, 'disga_params_%07d' % step))
        disGB_param = fluid.dygraph.load_dygraph(os.path.join(dir, 'disgb_params_%07d' % step))
        disLA_param = fluid.dygraph.load_dygraph(os.path.join(dir, 'disla_params_%07d' % step))
        disLB_param = fluid.dygraph.load_dygraph(os.path.join(dir, 'dislb_params_%07d' % step))

        self.genA2B.load_dict(genA2B_param)
        self.genB2A.load_dict(genB2A_param)
        self.disGA.load_dict(disGA_param)
        self.disGB.load_dict(disGB_param)
        self.disLA.load_dict(disLA_param)
        self.disLB.load_dict(disLB_param)
